...
# local imports
from .serializers import EmailCodeSerializer, UserRegisterSerializer
...
from utils.verification import cache_email_code, generate_code
        ...
        responses={
            ...
            400: inline_serializer(
                ...
                fields={
                    "field_name": CharField(
                        default="user with this {field_name} already exists."
                    ),
                },
            ),
            401: inline_serializer(
                ...
                fields={
                    ...
                    "errors": ListField(child=CharField()),
                },
            ),
        },
    )
...
class UserRegisterView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"User and cart created successfully. username: {username} user_id: {user.pk}"
            )
...
class VerifyEmail(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.is_email_verified:
            logger.info(
                f"A verified user requested a verification code. user_id: {user.pk}"
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.is_email_verified:
            logger.info(
                f"A verified user attempted to verify their email. username: {user.username}"
            )
            return Response(status=200, data={"message": "Email is already verified"})
        ...
        if cache.get(f"verify_email_{user.id}") == code:
            ...
            return Response(
                status=200, data={"message": "Email has been verified successfully"}
            )
        else:
