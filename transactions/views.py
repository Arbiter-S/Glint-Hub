    ...
    get=extend_schema(
        ...
        responses={
            200: inline_serializer(
                "Gateway URL",
                {
                    "status_code": IntegerField(max_value=100),
                    "url": CharField(
...
class PaymentInitiatorView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if order.total_price != current_price:
            ...
            return Response(
                {"Message": "Order price has been updated. Please try again."},
        ...
        response = requests.post(
            "https://sandbox.zarinpal.com/pg/v4/payment/request.json", json=body
        )
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"Payment gateway successfully initiated. transaction id: {transaction.id}, order id: {order.id}"
            )
            return Response(
                {
                    "status_code": code,
            ...
            logger.error(
                f"Payment gateway failed to initiate for an order. order id: {order.id}, error: {error}"
            )
            return Response({"status": "something went wrong", "errors": error})
...
class PaymentVerifierView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if status == "OK":
            ...
            response = requests.post(
                "https://sandbox.zarinpal.com/pg/v4/payment/verify.json", json=body
            )
            response_json = response.json()
            code = response_json["data"]["code"]
