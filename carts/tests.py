...
def test_cart_retrieve_success(add_products_to_cart):
    ...
    for i in range(len(selected_products)):
        ...
        assert (
            response["items"][i]["product"]["description"]
            == selected_products[i][0].description
        )
        assert float(response["items"][i]["product"]["weight"]) == float(
            selected_products[i][0].weight
        )
        assert response["items"][i]["product"]["color"] == selected_products[i][0].color
        assert (
            response["items"][i]["product"]["in_stock"]
            == selected_products[i][0].in_stock
        )
        assert (
            response["items"][i]["product"]["category"]
            == selected_products[i][0].category
        )
